...
export default function FlagsPage() {
  ...
  async function handleKillSwitch() {
    if (!confirm('VARNING: Detta kommer att inaktivera ALLA feature flags omedelbart. Är du helt säker?')) return
    try {
      const res = await fetch('/api/admin/flags/kill-all', { method: 'POST' })
      const data = await res.json()
      if (data.success) {
        setFlags(prev => prev.map(f => ({ ...f, enabled: false })))
        alert('Alla feature flags har inaktiverats.')
      }
    } catch (error) {
      console.error('Failed to kill all flags:', error)
      alert('Det gick inte att inaktivera flaggor.')
    }
  }
        ...
        <div className="flex items-center gap-3">
          {flags.filter(f => f.enabled).length > 0 && (
            <button
              ...
              title="Inaktivera alla flags omedelbart"
            >
