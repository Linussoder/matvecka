...
// Helper function to count users based on filters
async function countUsersForFilters(filters) {
  try {
...
export async function GET() {
  try {
    ...
    // Update user counts for dynamic segments
    const updatedSegments = await Promise.all(
      segments.map(async (segment) => {
        if (segment.is_dynamic && segment.filters) {
          const userCount = await countUsersForFilters(segment.filters)
...
export async function POST(req) {
  try {
    ...
    const userCount = await countUsersForFilters(filters || {})
    ...
    return NextResponse.json({ success: true, segment: { ...segment, user_count: userCount } })
  } catch (error) {
